...
export default function LoginPage() {
  ...
  // Facebook + Zalo OAuth tạm tắt theo chính sách. Route + lib của cả hai
  // vẫn còn (facebook-oauth.ts, zalo-oauth.ts, /api/auth/facebook/*,
  // /api/auth/zalo/*) — chỉ ẩn button. Bật lại = paste block JSX nút tương
  // ứng từ commit 6307acf.
  const googleUrl = `/api/auth/google?redirect=${encodeURIComponent(safeRedirect)}${ref ? `&ref=${encodeURIComponent(ref)}` : ""}`;
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#fcfcfd] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-[#e8e8ec] bg-white p-7 shadow-sm">
          ...
          {/* Primary path: Google. One click for both signin and signup. */}
          <a
            href={googleUrl}
