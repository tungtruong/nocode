    ...
    <div className="flex min-h-screen items-center justify-center bg-[#fcfcfd] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-[#e8e8ec] bg-white p-7 shadow-sm">
          ...
          {/* Primary: Google. Same /api/auth/google endpoint as login —
              first sign-in creates the account, returning user just logs in.
              Facebook + Zalo OAuth tạm tắt theo chính sách — route + lib
              (facebook-oauth.ts, zalo-oauth.ts, /api/auth/facebook/*,
              /api/auth/zalo/*) vẫn còn để re-enable nhanh khi cần. */}
          <a
            href={`/api/auth/google?redirect=/builder${ref ? `&ref=${encodeURIComponent(ref)}` : ""}`}
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-[#e8e8ec] bg-white py-2.5 text-sm font-medium text-[#334155] hover:bg-[#f8fafc] hover:border-[#cbd5e1] transition-all shadow-sm mb-5"
          >
