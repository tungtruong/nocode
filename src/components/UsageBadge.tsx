...
export function UsageBadge({ refreshKey }: { refreshKey?: number }) {
  ...
  // Tone: green <70%, amber 70-89%, red ≥90%.
  const tone =
  ...
  const tierLabel = info.tier === "pro" ? "Pro" : info.tier === "team" ? "Team" : t.planFree as string;

  return (
    <div
      className={`flex items-center gap-1.5 sm:gap-2 rounded-full border ${tone.ring} pl-2 sm:pl-2.5 pr-2.5 sm:pr-3 py-1`}
      title={`${t.quotaTier}: ${tierLabel} · ${info.used.toLocaleString()} / ${info.quota.toLocaleString()} ${t.quotaTokens} · ${t.quotaResetMonthly}`}
    >
      {/* Tier label hides on very narrow screens; the bar + numbers stay. */}
